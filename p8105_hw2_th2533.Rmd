...
* There are `r round(transit_df %>% filter(vending == "NO") %>% distinct(station_name, line, entry) %>% filter(entry == 1) %>% count()/transit_df %>% filter(vending == "NO") %>% distinct(station_name, line, entry) %>% count(),2)` of station entrances / exits without vending allow entrance
