...
our final dataset. The entry variable has been converted from a
character to a logical variable. This data is not tidy because we have
many columns for route information.

  - There are 465 distinct stations in NYC.
  - Among those, there are 84.
  - There are \`r transit\_df %\>% filter(vending == “YES”) %\>%
    distinct(station\_name, line,entry) %\>% count(entry)
